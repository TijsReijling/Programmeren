...
There were some annoying issues along the way, like missing values, misaligned categories, or weird formatting, but we handled those as they came up. We solved many of them by converting text into numbers, standardising names, or taking averages while skipping missing values.
...
```{r amsterdam_rotterdam_plot, echo=FALSE, fig.width=6, fig.height=3}
ggplot(Ervaren_Gezondheid, 
       aes(x = Perioden, y = ErvarenGezondheidGoedZeerGoed_4,
           colour = Gemeentenaam_1, group = Gemeentenaam_1)) +
  geom_line(size = 1.5) +
...
```{r national_plot, echo=FALSE, fig.width=6, fig.height=3}
ggplot(Ervaren_Gezondheid_NL_AM_RO, 
  ...
  labs(
    x = "Year (2012, 2016, 2020)",
    y = "Perceived Health (% Well or Very Well)",
...
The prevalence of purple on the map indicates a moderate positive correlation between income and experienced health in most municipalities. Although it should be noted that correlation does not equal causation, we can confirm there exists a relation between income and health.

One limitation of this map is the existence of grey areas that could not be computed due to a lack of data. In general, correlations are more accurate when they are based of a larger dataset. Therefore, we considered zooming the map out and display it at the provincial level. However, we decided to not go with this approach, because it would lead to underrepresentation of urban areas by assigning them the same weight as less populated municipalities within each province. This issue could be solved by assigning weight to municipalities, but this would over complicate things. Additionally, keeping the map at the municipal level allows for a clearer visualisation of contrast between urban and rural areas.
...
Despite the Dutch government’s commitment to universal healthcare and equal access to services, substantial differences in life expectancy and perceived health persist between socioeconomic groups. In our sub-population analysis we found that individuals with higher levels of education report higher well-being. We also found that the well-being of primary level educated people fluctuates more, as shown in our graph from 2015–2018, because of their vulnerable financial stability. Our spatial analysis found that the socioeconomic inequalities regarding income are more prevalent in urban areas as opposed to rural areas. We also saw how programs like “Vitaal en Healthy”, launched by the municipality of Rotterdam, contributed to a higher reported experienced health amongst its citizens. This suggests that government initiatives can indeed help reduce this health inequality.

Together, these findings paint a consistent picture: both income and education are strongly linked to differences in health and well-being in the Netherlands. While national health policies remain essential, they are not sufficient to tackle disparities on their own. The persistent regional and socioeconomic disparities call for targeted, localized approaches. Effective programs must be constructed in order to tackle this socioeconomic problem. By integrating health, education, and social policy, and investing particularly in vulnerable communities, we can create more equitable conditions. To allow all citizens, regardless of income or background, to live healthier lives
